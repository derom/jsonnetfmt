...
class Formatter(object):
	...
	def __init__(self, view):
		...
		self.cmd = settings.get('cmd', ['jsonnet', 'fmt', '-'])
	...
	def _show_errors(self, errors):
		...
		'''
		panel = self.window.create_output_panel('jsonnetfmt')
		panel.set_scratch(True)
		panel.run_command('select_all')
		panel.run_command('right_delete')
		panel.run_command('insert', {'characters': ANSI_ESCAPE.sub('', errors)})
		self.window.run_command('show_panel', {'panel': 'output.jsonnetfmt'})

	def _hide_errors(self):
		'''
		Hide any previously displayed error panel.
		'''
		self.window.run_command('hide_panel', {'panel': 'output.jsonnetfmt'})

class JsonnetfmtCommand(sublime_plugin.TextCommand):
	'''
	The `jsonnetfmt` command, invoked by the command palette or before a file save.
	'''
	def is_enabled(self):
		'''
		Checks if the current file viewed is a jsonnet file (.jsonnet, .libsonnet) or not.

		:returns: bool
		'''
		return self.view.score_selector(0, 'source.jsonnet') != 0
...
class JsonnetfmtListener(sublime_plugin.EventListener):
	def on_pre_save(self, view):
		'''
		Called before a dirty view is stored to disk, running the `jsonnetfmt` command.
		'''
		if settings.get('format_on_save', True):
			view.run_command('jsonnetfmt')
